module Upright
  module Probes
    class HTTPProbe < FrozenRecord::Base
      include Probeable
      ...
      private
        def perform_request
          HTTP::Request.new(url, **request_options).get
        end
        ...
        def expected_status_range
          if expected_status.is_a?(Integer)
        ...
        def attach_verbose_log(probe_result)
          if last_response
            Artifact.new(name: "curl.log", content: last_response.verbose_log_content).attach_to(probe_result)
          end
        end

        def attach_response_body(probe_result)
          if last_response && last_response.body.present?
            Artifact.new(name: "response.#{last_response.file_extension}", content: last_response.body).attach_to(probe_result)
          end
        end
    end
  end
