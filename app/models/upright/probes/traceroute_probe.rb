module Upright
  module Probes
    class TracerouteProbe < FrozenRecord::Base
      include Probeable
      include Traceroute::OtelTracing
      ...
      private
        def attach_traceroute_json(probe_result, traceroute_result)
          if traceroute_result.raw_json.present?
            Artifact.new(name: "traceroute.json", content: traceroute_result.raw_json).attach_to(probe_result)
          end
        end
    end
  end
